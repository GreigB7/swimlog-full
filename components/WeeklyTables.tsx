...
// Keep English values in DB, show Dutch labels in UI
...
function toNLEffort(v?: string | null) {
  const hit = EFFORT_OPTIONS.find(o => o.value.toLowerCase() === (v || '').toLowerCase());
  return hit ? hit.label : (v || '');
}
...
export function WeeklyTables({ userId, canEdit, date }: { userId: string, canEdit: boolean, date: string }) {
  ...
  async function save() {
    ...
    const payload = { ...vals }; // vals already holds English codes for type/effort
              ...
              {train.map(r => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    {/* TYPE (NL labels; store ENG value) */}
                    <td>
                      {isEdit ? (
                        <select
                    ...
                    {/* EFFORT (NL labels; store ENG value) */}
                    <td>
                      {isEdit ? (
                        <select
