...
export function TrainingForm() {
  ...
  const [effort, setEffort] = useState('Green');
  ...
  return (
    <div className="card vstack">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div><div className="label">Effort</div><input value={effort} onChange={e=>setEffort(e.target.value)} /></div>
        <div><div className="label">Complexity (1-3)</div><input type="number" min={1} max={3} value={cx} onChange={e=>setCx(e.target.value ? parseInt(e.target.value) : '')} /></div>
      </div>
